...
method.process = function (params,cb) {
  ...
  self.searches.forEach( function (search, searchIdx) {
    ...
    en.getMessages(null, function (err, messages) {
      ...
      if (err) {
        log.error('[%s] [%s]: Error retrieving emails for search: %s\n%s ', mb, searchIdx, err.message, err.stack)
        cb(err)
        return null;
      }

      var numMessages = (messages)? messages.length : 0
      log.info('[%s] [%s] Found %s messages', self.name, searchIdx, numMessages ) ;
      ...
      messages.forEach( function(message,idx) {
        ...
        log.info('[%s] [%s] [%s]: Found message: "%s" (%s)', mb, searchIdx, id, subject, date) ;
      ...
      // Update labels
      if (actions.indexOf('applyProcessedLabel') >= 0 || actions.indexOf('markAsRead') >= 0) {
        ...
        log.info('[%s] [%s] Updating %s messages [%s %s]', mb, searchIdx, numMessages, (applyProcessedLabel)? "applyProcessedLabel" : "", (markAsRead)? "markAsRead" : "") ;
        ...
        en.updateLabels({
          ...
          if (err) {
            log.error('[%s] [%s] [%s]: Error updating labels: %s\n%s ', mb, searchIdx, resps, err.message, err.stack)
            cb(err)
            return null;
          }
          log.info('[%s] [%s] Updated %s messages', mb, searchIdx, numMessages) ;
        });

      }

      // Trash messages
      if (actions.indexOf('trash') >= 0) {
        en.trash(null, function (err,resps) {
          if (err) {
            log.error('[%s] [%s] [%s]: Error trashing message: %s\n%s ', mb, searchIdx, resps, err.message, err.stack)
            cb(err)
            return null;
          }
