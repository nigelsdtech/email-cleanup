...
method.process = function (params,cb) {
  ...
  self.searches.forEach( function (el, searchIdx) {
    ...
    self.gmail.listMessages({
      freetextSearch: el,
      maxResults: (cfg.maxSearchResults || 100),
      ...
      bf.each( function(idx,id,done) {
        ...
        self.gmail.getMessage({
          ...
          if (err) {
            log.error('[%s] [%s] [%s]: Error getting details for message: %s\n%s ', mb, searchIdx, id, err.message, err.stack)
            cb(err)
	    return null
          }
	  ...
	  var headers = resp.payload.headers
          ...
          log.info('[%s] [%s] [%s]: Trashing message: "%s" (%s)', mb, searchIdx, id, subject, date) ;
	  done(null)
	})
